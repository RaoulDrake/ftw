...
class PongDuel(gym.Env):
    ...
    def __init__(self, step_cost=0, reward=1, max_rounds=10):
        ...
        # ----------EDIT BY JOHANNES TOCHTERMANN:----------
        # action space was modified to spaces.Tuple, to enable compatability with dm_env.Environment wrappers.
        ...
        # ----------EDIT BY JOHANNES TOCHTERMANN:----------
        # The following 2 assignments were edited to ensure compatibility with higher gym versions.
        # Specifically, the arrays were cast to be of type np.float32.
        self._obs_low = np.array([0., 0., 0., 0.] + [0.] * len(BALL_DIRECTIONS)).astype(np.float32)
        self._obs_high = np.array([1., 1., 1., 1.] + [1.] * len(BALL_DIRECTIONS)).astype(np.float32)
        # self.observation_space = MultiAgentObservationSpace([spaces.Box(self._obs_low, self._obs_high)
        #                                                      for _ in range(self.n_agents)])
        # action space was modified to spaces.Tuple, to enable compatability with dm_env.Environment wrappers.
        self.observation_space = spaces.Tuple([spaces.Box(self._obs_low, self._obs_high)
                                               for _ in range(self.n_agents)])
    ...
    def get_agent_obs(self):
        ...
        # ----------EDIT BY JOHANNES TOCHTERMANN:----------
        # The observation is converted from a python list to a numpy array,
        # to ensure compatibility with dm-acme.
        _obs = np.array(_obs).astype(np.float32)
    ...
    def step(self, action_n):
        ...
        # ----------EDIT BY JOHANNES TOCHTERMANN:----------
        # done is now returned as a single boolean, indicating that both agents are done.
        # This does not change the original game logic.
        # Additionally, the original self._agent_dones array is returned as part of the info return value.
        return (self.get_agent_obs(), np.array(rewards).astype(np.float32), all(self._agent_dones),
                {'rounds': self.__rounds, 'agent_dones': self._agent_dones})

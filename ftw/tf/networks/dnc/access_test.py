...
class MemoryAccessTest(tf.test.TestCase):
    ...
    def testBuildAndTrain(self):
        ...
        with tf.GradientTape() as tape:
            output, _ = snt.dynamic_unroll(
                self.module, inputs, self.module.initial_state(BATCH_SIZE))

            targets = np.random.rand(TIME_STEPS, BATCH_SIZE, NUM_READS, WORD_SIZE)
            loss = tf.reduce_mean(tf.square(output - targets))
        gradient = tape.gradient(loss, self.module.trainable_variables)
        snt.optimizers.SGD(1).apply(gradient, self.module.trainable_variables)

    def testValidReadMode(self):
        inputs = self.module._read_inputs(
            tf.random.normal([BATCH_SIZE, INPUT_SIZE]))
        inputs = tree.map_structure(lambda t: t.numpy(), inputs)
    ...
    def testGradients(self):
        ...
        tensors_to_check = [
            inputs] + tree.map_structure(tf.convert_to_tensor, [
            self.initial_state.memory, self.initial_state.read_weights,
        ...
        @tf.function(
            input_signature=[tf.TensorSpec(shape=shape, dtype=tf.dtypes.float32) for shape in shapes])
        def get_loss(inputs_, memory, read_weights, precedence_weights, link):
            state = access.AccessState(
                memory=memory,
                read_weights=read_weights,
                write_weights=tf.convert_to_tensor(self.initial_state.write_weights),
                linkage=addressing.TemporalLinkageState(
                    link=link, precedence_weights=precedence_weights),
                usage=tree.map_structure(tf.convert_to_tensor, self.initial_state.usage))
            output, _ = self.module(inputs_, state)
            loss = tf.reduce_sum(output)
            return loss

        theoretical_grad, numerical_grad = tf.test.compute_gradient(
            f=get_loss,
            x=tensors_to_check, delta=1e-5)

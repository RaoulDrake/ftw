...
from ftw.tf.networks.dnc import addressing
from ftw.tf.networks.dnc import util
...
class WeightedSoftmaxTest(tf.test.TestCase):
    ...
    def testValues(self):
        ...
        observed = addressing.weighted_softmax(activations_data, weights_data, tf.identity)
        expected = tf.nn.softmax(activations_data)
...
class CosineWeightsTest(tf.test.TestCase):
    ...
    def testShape(self):
        ...
        mem = tf.zeros(dtype=tf.dtypes.float32, shape=[batch_size, memory_size, word_size])
        keys = tf.zeros(dtype=tf.dtypes.float32, shape=[batch_size, num_heads, word_size])
        strengths = tf.zeros(dtype=tf.float32, shape=[batch_size, num_heads])
    ...
    def testValues(self):
        ...
        module = addressing.CosineWeights(num_heads, word_size)
        weights = module(mem_data, keys_data, strengths_data)
    ...
    def testDivideByZero(self):
        ...
        self.assertFalse(np.any(np.isnan(gradients[0].numpy())))
        self.assertFalse(np.any(np.isnan(gradients[1].numpy())))
        self.assertFalse(np.any(np.isnan(gradients[2].numpy())))
...
class TemporalLinkageTest(tf.test.TestCase):
    ...
    def testModule(self):
        ...
        for i in range(num_steps):
            ...
            state = module(write_weights_in,
                           addressing.TemporalLinkageState(
                               link=prev_link_in,
                               precedence_weights=prev_precedence_weights_in))

        # link should be bounded in range [0, 1]
        self.assertGreaterEqual(state.link.numpy().min(), 0)
        self.assertLessEqual(state.link.numpy().max(), 1)
        ...
        self.assertLessEqual(state.link.numpy().sum(2).max(), 1)
        self.assertLessEqual(state.link.numpy().sum(3).max(), 1)

        # records our transitions in batch 0: head 0: 0->1, and head 1: 3->2
        self.assertAllEqual(state.link.numpy()[0, 0, :, 0], util.one_hot(memory_size, 1))
        self.assertAllEqual(state.link.numpy()[0, 1, :, 3], util.one_hot(memory_size, 2))
        ...
        forward_read_weights = module.directional_read_weights(
            tf.constant(state.link.numpy(), dtype=tf.float32),
        ...
        forward_read_weights = forward_read_weights.numpy()
        backward_read_weights = backward_read_weights.numpy()
    ...
    def testPrecedenceWeights(self):
        ...
        precedence_weights = precedence_weights.numpy()
...
class FreenessTest(tf.test.TestCase):
    ...
    def testWriteAllocationWeightsGradient(self):
        ...
        def write_allocation_weights_test_fn(usage_, write_gates_):
            return module.write_allocation_weights(usage_, write_gates_, num_writes)

        write_allocation_weights_fn = tf.function(
            write_allocation_weights_test_fn,
            input_signature=[
                tf.TensorSpec(shape=usage.shape, dtype=usage.dtype),
                tf.TensorSpec(shape=write_gates.shape, dtype=write_gates.dtype)])

        theoretical_grad, numerical_grad = tf.test.compute_gradient(
            f=write_allocation_weights_fn,
            x=[usage, write_gates], delta=1e-5)
        err = util.compute_error(theoretical_grad, numerical_grad)
    ...
    def testAllocationGradient(self):
        ...
        theoretical_grad, numerical_grad = tf.test.compute_gradient(
            f=tf.function(module._allocation), x=[usage], delta=1e-5)

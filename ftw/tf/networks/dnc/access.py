...
class MemoryAccess(snt.RNNCore):
    ...
    *   `addressing.FreenessAllocator` for keeping track of memory usage, where
    ...
    def _initialise(self):
        ...
        self._write_vectors = snt.Linear(self._num_writes * self._word_size, name='write_vectors')

        # e_t^i - Amount to erase the memory by before writing, for each write head.
        self._erase_vectors = snt.Linear(self._num_writes * self._word_size, name='erase_vectors')

        # f_t^j - Amount that the memory at the locations read from at the previous
        # time step can be declared unused, for each read head `j`.
        self._free_gate = snt.Linear(self._num_reads, name='free_gate')

        # g_t^{a, i} - Interpolation between writing to unallocated memory and
        # content-based lookup, for each write head `i`. Note: `a` is simply used to
        # identify this gate with allocation vs writing (as defined below).
        self._allocation_gate = snt.Linear(self._num_writes, name='allocation_gate')

        # g_t^{w, i} - Overall gating of write amount for each write head.
        self._write_gate = snt.Linear(self._num_writes, name='write_gate')
        ...
        self._read_mode = snt.Linear(self._num_reads * num_read_modes, name='read_mode')

        # Parameters for the (read / write) "weights by content matching" modules.
        self._write_keys = snt.Linear(self._num_writes * self._word_size, name='write_keys')
        self._write_strengths = snt.Linear(self._num_writes, name='write_strengths')

        self._read_keys = snt.Linear(self._num_reads * self._word_size, name='read_keys')
        self._read_strengths = snt.Linear(self._num_reads, name='read_strengths')
    ...
    def _read_inputs(self, inputs):
        ...
        def _linear(linear_layer, first_dim, second_dim, name, activation=None):
        ...
        # v_t^i - The vectors to write to memory, for each write head `i`.
        write_vectors = _linear(
            self._write_vectors, self._num_writes, self._word_size, 'write_vectors')

        # e_t^i - Amount to erase the memory by before writing, for each write head.
        erase_vectors = _linear(
            self._erase_vectors, self._num_writes, self._word_size, 'erase_vectors',
            tf.sigmoid)

        # f_t^j - Amount that the memory at the locations read from at the previous
        # time step can be declared unused, for each read head `j`.
        free_gate = tf.sigmoid(self._free_gate(inputs))

        # g_t^{a, i} - Interpolation between writing to unallocated memory and
        # content-based lookup, for each write head `i`. Note: `a` is simply used to
        # identify this gate with allocation vs writing (as defined below).
        allocation_gate = tf.sigmoid(self._allocation_gate(inputs))

        # g_t^{w, i} - Overall gating of write amount for each write head.
        write_gate = tf.sigmoid(self._write_gate(inputs))

        # \pi_t^j - Mixing between "backwards" and "forwards" positions (for
        # each write head), and content-based lookup, for each read head.
        num_read_modes = 1 + 2 * self._num_writes
        read_mode = snt.BatchApply(tf.nn.softmax)(_linear(
            self._read_mode, self._num_reads, num_read_modes, name='read_mode'))

        # Parameters for the (read / write) "weights by content matching" modules.
        write_keys = _linear(
            self._write_keys, self._num_writes, self._word_size, 'write_keys')
        write_strengths = self._write_strengths(inputs)

        read_keys = _linear(
            self._read_keys, self._num_reads, self._word_size, 'read_keys')
    ...
    def _read_weights(self, inputs, memory, prev_read_weights, link):
        ...
        with tf.name_scope(
            ...
            read_weights = (tf.expand_dims(content_mode, 2) * content_weights +
    ...
    def initial_state(self, batch_size: int, **unused_kwargs):
        return AccessState(
            memory=tf.zeros(shape=(batch_size, self._memory_size, self._word_size),
                            dtype=tf.float32),
            read_weights=tf.zeros(shape=(batch_size, self._num_reads, self._memory_size),
                                  dtype=tf.float32),
            write_weights=tf.zeros(shape=(batch_size, self._num_writes, self._memory_size),
                                   dtype=tf.float32),

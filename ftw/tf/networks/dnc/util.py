...
def reduce_prod(x, axis, name=None):
    ...
    Uses tf.cumprod and tf.gather_nd as a workaround to the
    poor performance of calculating tf.reduce_prod's gradient on CPU.
    """
...
def compute_error(theoretical_grad, numerical_grad):
    error = 0
    for j_t, j_n in zip(theoretical_grad, numerical_grad):
